...
3. (no repo do modelo, `atos-de-fala`) semear itens a anotar:
   `DATABASE_URL=... python -m atos.collect export --dataset data/dataset.jsonl --honeypots gold/honeypots.jsonl`
4. `DATABASE_URL=... npm run dev`        # http://localhost:3000

## Deploy (Vercel)
1. Projeto na Vercel apontando pra este repo (root = raiz do repo; já linkado, push = deploy).
2. Setar `DATABASE_URL` (Neon) nas env vars da Vercel.
3. Rodar a migration uma vez apontando pro Neon de produção:
...
confiabilidade) e análise de percepção fica no pacote Python `atos.collect`, que lê/escreve
o mesmo Postgres. `lib/scoring.ts` é espelho exato de `src/atos/collect/score.py` —
